...
public class MovementAdapter extends KeyAdapter {
    ...
    public void moveAdventurer(Coordinates vector) {
        ...
        List<Coordinates> coordlist = gameWorld.getAdventurer().getLocation().findAllSafeMoves(gameWorld.getAdventurer().getWorld());
        for(Coordinates vct : coordlist){
            if(vector.equals(vct)){
                moveAdventurer(vector);
            }
        }
        moveAdventurer(Coordinates.NORTH_VECTOR);
